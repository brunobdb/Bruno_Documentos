CREATE PROCEDURE PROC_CRAWLING_AP1_TABLET_PT2
LANGUAGE SQLSCRIPT as
BEGIN
-- Etapa 5
	TRUNCATE TABLE OW_LAO.TF_CRAWLING_AP1_TABLET_6;
					
	INSERT INTO OW_LAO.TF_CRAWLING_AP1_TABLET_6
	SELECT 
...
FROM (
	...
	FROM OW_LAO.TF_CRAWLING_AP1_TABLET_5			
...
;
-- Etapa 6
TRUNCATE TABLE OW_LAO.TF_CRAWLING_AP1_TABLET_7;
	INSERT INTO OW_LAO.TF_CRAWLING_AP1_TABLET_7
	SELECT 
		rsn_modelnm,
		idxbyrsnmodelnm,
		model_code,
	...
	FROM  (
		...
		FROM (
			...
			*/
			...
			FROM 
			OW_LAO.TF_CRAWLING_AP1_TABLET_6
			WHERE fix_ranking=1
...
-- Etapa 7
TRUNCATE TABLE OW_LAO.TF_CRAWLING_AP1_TABLET_8;
INSERT INTO OW_LAO.TF_CRAWLING_AP1_TABLET_8
SELECT 
	...
	a.*	
FROM OW_LAO.TF_CRAWLING_AP1_TABLET_2 a
INNER JOIN OW_LAO.TF_CRAWLING_AP1_TABLET_7 b
--20190611 모델명 통일 삭제
--on a.model_nm = b.rsn_modelnm
ON a.model_nm = b.rsn_modelnm AND a.idxbyrsnmodelnm = b.idxbyrsnmodelnm
INNER JOIN OW_LAO.MP_GSMARENA_MERGE_DATA_MAPPING c
...
SELECT 
	DISTINCT
	SITE_URL
FROM OW_LAO.TF_CRAWLING_AP1_TABLET_8 
WHERE
...
FROM OW_LAO.TF_CRAWLING_AP1_TABLET_1 F
LEFT JOIN OW_LAO.TF_CRAWLING_AP1_TABLET_8 T 
ON F.SITE_URL = T.SITE_URL 
WHERE
T.SITE_URL is null AND F.PROCESS_STATUS is null
);
-- Etapa 8
TRUNCATE TABLE OW_LAO.TF_CRAWLING_AP1_TABLET_9;
INSERT INTO OW_LAO.TF_CRAWLING_AP1_TABLET_9
	SELECT * FROM OW_LAO.TF_CRAWLING_AP1_TABLET_8;
  
MERGE INTO OW_LAO.TF_CRAWLING_AP1_TABLET_9 O
USING OW_LAO.MP_GSMARENA_MERGE_DATA_A_MINMODELCODE M 
...
TRUNCATE TABLE OW_LAO.TF_CRAWLING_AP1_TABLET_10;
INSERT INTO OW_LAO.TF_CRAWLING_AP1_TABLET_10
SELECT * FROM OW_LAO.TF_CRAWLING_AP1_TABLET_9;
MERGE INTO OW_LAO.TF_CRAWLING_AP1_TABLET_10 O
USING (
	SELECT min(pa_model_code) as pa_model_code, pa_mktcode, pa_brand_nm, pa_model_nm, COUNT(1) FROM 
		(SELECT distinct pa_model_code, pa_mktcode, pa_brand_nm, pa_model_nm FROM OW_LAO.TF_CRAWLING_AP1_TABLET_10) v
...
-- Etapa 9
TRUNCATE TABLE OW_LAO.TF_CRAWLING_AP1_TABLET_11;
-- TO_TIMESTAMP ('2010-01-11 13:30:00', 'YYYY-MM-DD HH24:MI:SS') 
INSERT INTO OW_LAO.TF_CRAWLING_AP1_TABLET_11
SELECT
	...
	*/
	...
	FROM (
		...
		FROM (
			...
			FROM
			(
				SELECT * FROM OW_LAO.TF_CRAWLING_AP1_TABLET_10
			...
			(
				SELECT 
					...
					price_unit,
					count(1) as t_cnt
				FROM OW_LAO.TF_CRAWLING_AP1_TABLET_10
				--WHERE price_local_base > 0 
...
;
-- Etapa 10
TRUNCATE TABLE OW_LAO.TF_CRAWLING_AP1_TABLET_12;
insert into OW_LAO.TF_CRAWLING_AP1_TABLET_12
(
"MODEL_CODE",
"BRAND_NM",
"MKTCODE",
...
UPPER(VENDOR.VENDOR),
MEDIAN.MEDIAN
FROM OW_LAO.VW_CRAWLING_ONLINE_PRICE_TABLET VW_PRICE --Era VW_CRAWLING_ONLINE_PRICE Criei a VW_CRAWLING_ONLINE_PRICE_TABLET
left join (
select count(1), PROD_HREF, VENDOR
from "OW_LAO"."ODS_CRAWLING_AP1_TABLET_PRODUCT_DETAIL" --Mudar em Prod
...
INSERT INTO FT_CRAWLING_ONLINE_PRICE_DISPLAY_SHARE_TABLET
SELECT *, NULL REFER_WEEK, NULL REFER_WEEK_T, NULL REFER_YEAR FROM "OW_LAO"."VW_CRAWLING_ONLINE_PRICE_DISPLAY_SHARE_TABLET";
--################################
UPDATE OW_LAO.FT_CRAWLING_ONLINE_PRICE_DISPLAY_SHARE_TABLET F
SET
...
WHERE 
REFER_WEEK is null;
--################################ Não tira o Comentário desse bloco
/*
...
*/
--################################
DELETE FROM FT_CRAWLING_ONLINE_PRICE_SEGMENT_TABLET
WHERE 1=1
AND to_char("REFER_DATE", 'YYYY-MM-DD')  BETWEEN to_char(add_days(CURRENT_DATE,-15), 'YYYY-MM-DD') and  to_char(CURRENT_DATE, 'YYYY-MM-DD');
--and MKTNAME is not null ;
INSERT INTO FT_CRAWLING_ONLINE_PRICE_SEGMENT_TABLET
SELECT *, NULL REFER_WEEK, NULL REFER_WEEK_T, NULL REFER_YEAR,NULL AS "MODEL_DESC" FROM "OW_LAO"."TF_CRAWLING_AP1_TABLET_12";
--################################
UPDATE OW_LAO.FT_CRAWLING_ONLINE_PRICE_SEGMENT_TABLET F
SET
REFER_WEEK = (SELECT DISTINCT YYYYWW FROM OW_MD.DIM_CALENDAR D WHERE to_char(F.REFER_DATE,'YYYY-MM-DD') = to_char(D.YYYYMMDD,'YYYY-MM-DD'))
, REFER_WEEK_T = ( SELECT DISTINCT 'W' || RIGHT(YYYYWW,2) FROM OW_MD.DIM_CALENDAR D WHERE to_char(F.REFER_DATE,'YYYY-MM-DD') = to_char(D.YYYYMMDD,'YYYY-MM-DD'))
, REFER_YEAR = ( SELECT DISTINCT  YYYY FROM OW_MD.DIM_CALENDAR D WHERE to_char(F.REFER_DATE,'YYYY-MM-DD') = to_char(D.YYYYMMDD,'YYYY-MM-DD'))
...
REFER_WEEK is null;
--################################
/*01/10/2021 - Carregar a DisplayShare na Segment Chamado SDSMS-2112*/
/*
delete from 
"OW_LAO"."FT_CRAWLING_ONLINE_PRICE_SEGMENT_TABLET"
...
insert into   "OW_LAO"."FT_CRAWLING_ONLINE_PRICE_SEGMENT_TABLET" (
"MODEL_CODE",
"BRAND_NM",
"MKTCODE",
"MKTNAME",
...
"REFER_WEEK_T",
"REFER_YEAR"
 from "OW_LAO"."FT_CRAWLING_ONLINE_PRICE_DISPLAY_SHARE_TABLET"  FT_SHARE
where to_date(to_char(FT_SHARE."REFER_DATE", 'YYYY-MM-DD'), 'YYYY-MM-DD')  IN(
select distinct 
 to_date(to_char("REFER_DATE", 'YYYY-MM-DD') , 'YYYY-MM-DD')
 from "OW_LAO"."FT_CRAWLING_ONLINE_PRICE_SEGMENT_TABLET"
 where to_date(to_char("REFER_DATE", 'YYYY-MM-DD') , 'YYYY-MM-DD')=to_date(to_char(add_days(CURRENT_DATE,-1), 'YYYY-MM-DD') , 'YYYY-MM-DD')
 )
and SITE_URL not in ( 
select SITE_URL
from "OW_LAO"."FT_CRAWLING_ONLINE_PRICE_SEGMENT_TABLET"
...
/*
UPDATE   "OW_LAO"."FT_CRAWLING_ONLINE_PRICE_SEGMENT_TABLET" O
 SET
O."MODEL_DESC" =T."DESC" 
from 
 "OW_LAO"."FT_CRAWLING_ONLINE_PRICE_SEGMENT_TABLET"  O 
inner join 
 (select "DESC",PROD_HREF from "OW_LAO"."ODS_CRAWLING_AP1_TABLET_PRODUCT_DETAIL")T
...
--################################
/*
DELETE FROM FT_CRAWLING_ONLINE_PRICE_VS_COMPETITOR_TABLET
WHERE 1=1
AND to_char("REFER_DATE", 'YYYY-MM-DD')  BETWEEN to_char(add_days(CURRENT_DATE,-15), 'YYYY-MM-DD') and  to_char(CURRENT_DATE, 'YYYY-MM-DD');
insert into  "OW_LAO"."FT_CRAWLING_ONLINE_PRICE_VS_COMPETITOR_TABLET"
(
"SUBSIDIARY",
"COUNTRY_CODE",
"BRAND_NM",
...
"VENDOR",
"COMPETITOR_VENDOR"
 from "OW_LAO"."VW_CRAWLING_ONLINE_PRICE_VS_COMPETITOR_TABLET" ;
*/
END;