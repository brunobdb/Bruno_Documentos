CREATE PROCEDURE PROC_CRAWLING_AP1_TV_PT1
LANGUAGE SQLSCRIPT as
BEGIN
TRUNCATE TABLE TF_CRAWLING_AP1_TV_1;
INSERT INTO TF_CRAWLING_AP1_TV_1
SELECT DISTINCT
...
INNER JOIN "ODS_CRAWLING_AP1_CE_PRODUCT_DETAIL" D
ON P."KEY" = D."KEY"
LEFT JOIN "ODS_CRAWLING_AP1_TV_PRODUCT_DETAIL_DAILY" DT
...
from "OW_LAO"."TF_CRAWLING_AP1_TV_1" MX
		inner join (
					select 
					...
					,length (RETAIL_PRICE_ACTUAL) as "LIVER_ACT_LEGTH",RETAIL_PRICE_ACTUAL as "LIVER_RETAIL_PRICE_ACTUAL"
							from "OW_LAO"."TF_CRAWLING_AP1_TV_1"
...
;
*/
TRUNCATE TABLE TF_CRAWLING_AP1_TV_2;
INSERT INTO TF_CRAWLING_AP1_TV_2
SELECT
...
CALL PROC_CRAWLING_AP1_TV_SITE_MAPPING_MERGE_AUTOMATIC;
 TRUNCATE TABLE FT_CRAWLING_AP1_TV_DISPLAY_SHARE;
 
 INSERT INTO FT_CRAWLING_AP1_TV_DISPLAY_SHARE
 SELECT * 
 FROM VW_CRAWLING_AP1_TV_DISPLAY_SHARE
 ;
 
 TRUNCATE TABLE FT_CRAWLING_AP1_TV_SEGMENT;
 
  /*Campo "GFK_BrandList" adicionado por causa do Jira SDSMS-1097 criado pelo e.koeke */  
 INSERT INTO FT_CRAWLING_AP1_TV_SEGMENT                            
     SELECT A.* 
, NULL REFER_WEEK
, NULL REFER_WEEK_T
, NULL REFER_YEAR,
  CASE WHEN B.GFK_BRAND_TV IS NULL THEN 
            'NO'
       ELSE 'YES'
  END AS "GFK_BrandList"
 FROM "OW_LAO".VW_CRAWLING_AP1_TV_SEGMENT A
	LEFT JOIN "OW_LAO"."DIM_GFK_BRAND_TV" B ON (TRIM(A."UPPER(BRAND_NM)")= TRIM(B.GFK_BRAND_TV));
 
 UPDATE OW_LAO.FT_CRAWLING_AP1_TV_SEGMENT F
SET
...
REFER_WEEK is null;
update
"OW_LAO"."FT_CRAWLING_AP1_TV_SEGMENT"
