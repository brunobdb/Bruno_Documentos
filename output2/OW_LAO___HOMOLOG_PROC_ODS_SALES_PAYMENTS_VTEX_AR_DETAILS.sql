CREATE PROCEDURE OW_LAO.HOMOLOG_PROC_ODS_SALES_PAYMENTS_VTEX_AR_DETAILS
			AS
			BEGIN
			
			TRUNCATE TABLE OW_LAO.TMP_SALES_PAYMENTS_VTEX_AR_DETAILS_HOMOLOG;
		
INSERT INTO OW_LAO.TMP_SALES_PAYMENTS_VTEX_AR_DETAILS_HOMOLOG
SELECT DISTINCT
	NULL AS ID,
...
WHERE 	A.CLIENT_SUBSIDIARY_ID = 1
  ...
  --AND PO_ORDERID = '1509833696420-01'
...
------------------DEDUP  LAST UPDATE--------------
  DELETE 
        FROM OW_LAO.TMP_SALES_PAYMENTS_VTEX_AR_DETAILS_HOMOLOG
       WHERE DEDUP != 1     
  ;
 
 -----Update sales payments 
		UPDATE OW_LAO.TMP_SALES_PAYMENTS_VTEX_AR_DETAILS_HOMOLOG A
	SET A.REVENUE_LOCAL = B.VALUE  / 100 ,
	    A.PE_AMOUNT     = B.VALUE  / 100 
	
		FROM OW_LAO.TMP_SALES_PAYMENTS_VTEX_AR_DETAILS_HOMOLOG A
        JOIN U_PRJ_ECOM.RAW_VTEX_SSG_AR_SALES_ORDER_PAYMENT B ON B.ORDER_ID = A.PAY_ORDERID 
 ; 
 ...
 ; 
---------------Update ODS_PAYMENT_FUNNEL for VTEX AR Payments
        merge into 
        		ow_lao.ODS_PAYMENT_FUNNEL_HOMOLOG a
			using ow_lao.TMP_SALES_PAYMENTS_VTEX_AR_DETAILS_HOMOLOG
			b on b.pay_orderid    = a.pay_orderid
