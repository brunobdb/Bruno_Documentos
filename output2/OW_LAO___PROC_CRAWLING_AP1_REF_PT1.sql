CREATE PROCEDURE PROC_CRAWLING_AP1_REF_PT1
LANGUAGE SQLSCRIPT as
BEGIN
TRUNCATE TABLE TF_CRAWLING_AP1_REF_1;
INSERT INTO TF_CRAWLING_AP1_REF_1
SELECT DISTINCT
TRIM(UPPER(SUBSTRING(D."PROD_BRAND",0,99))) "brand_nm"
, SUBSTRING(D."MODEL",0,99) "model_nm" --ref1
...
, SUBSTRING(DR."RATING",0,99) "site_rating"
, SUBSTRING(DR."RATING_AMOUNT",0,99) "site_rating_amount"
, SUBSTRING(DR."RATING_SCALE",0,99) "site_rating_scale"
, SUBSTRING(P."TODAY_DATE_TIME",0,99) "refer_date"
, SUBSTRING(P."POR_VALUE",0,16) "price_unit"
, SUBSTRING(D."MODEL2",0,99) MODEL2 --ref3
, SUBSTRING(D."REF",0,99)  REF  --ref2
, SUBSTRING(D."REF2",0,99)  REF2  --ref4
...
from "OW_LAO"."TF_CRAWLING_AP1_REF_1" MX
		inner join (
					select 
...
----FIM do CODIGO PROVISORIO 
 ...
 ;
-- caso os X estão nulos colocamos vazio para mapear na query final da pt2
 UPDATE TF_CRAWLING_AP1_REF_1
 SET
...
;
*/
TRUNCATE TABLE TF_CRAWLING_AP1_REF_2;
INSERT INTO TF_CRAWLING_AP1_REF_2
SELECT
...
CALL PROC_CRAWLING_AP1_REF_SITE_MAPPING_MERGE_AUTOMATIC;
TRUNCATE TABLE FT_CRAWLING_AP1_REF_DISPLAY_SHARE;
 
 INSERT INTO FT_CRAWLING_AP1_REF_DISPLAY_SHARE
 SELECT * 
 FROM VW_CRAWLING_AP1_REF_DISPLAY_SHARE
 ;
TRUNCATE TABLE FT_CRAWLING_AP1_REF_SEGMENT;
 
 /*Campo "GFK_BrandList" adicionado por causa do Jira SDSMS-1097 criado pelo e.koeke */   
 INSERT INTO FT_CRAWLING_AP1_REF_SEGMENT                            
     SELECT A.* 
, NULL REFER_WEEK
, NULL REFER_WEEK_T
, NULL REFER_YEAR,
  CASE WHEN B.GFK_BRAND_REF IS NULL THEN 
            'NO'
       ELSE 'YES'
  END AS "GFK_BrandList"
 FROM "OW_LAO".VW_CRAWLING_AP1_REF_SEGMENT A
	LEFT JOIN "OW_LAO"."DIM_GFK_BRAND_REF" B ON (TRIM(A."UPPER(BRAND_NM)")= TRIM(B.GFK_BRAND_REF));
 
 
 
update
"OW_LAO"."FT_CRAWLING_AP1_REF_SEGMENT"
set "SITE_RATING" = "SITE_RATING" /1000000000000
where length ("SITE_RATING" )> 11;
 
 UPDATE OW_LAO.FT_CRAWLING_AP1_REF_SEGMENT F
SET
