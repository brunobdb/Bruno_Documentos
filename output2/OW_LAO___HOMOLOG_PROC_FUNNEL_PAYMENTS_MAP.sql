CREATE PROCEDURE OW_LAO.HOMOLOG_PROC_FUNNEL_PAYMENTS_MAP(
...
BEGIN
	...
	BEGIN 
		MERGE INTO 
		 	OW_LAO.DIM_PAYMENT_STATUS_MAPPING		AS A
		USING 
			OW_LAO.TMP_PAYMENT_STATUS_MAPPING_MINIO 	AS B 
		ON 
			A.PAY_STATUS			= B.PAY_STATUS
			AND A.PO_STATUS 		= B.PO_STATUS
			AND A.PAY_DESCRIPTION  	= B.PAY_DESCRIPTION
		WHEN MATCHED AND A.STATUS <> B.STATUS OR (A.ACTIVE = FALSE OR A.ACTIVE IS NULL) THEN UPDATE SET 
			A.STATUS				= B.STATUS,
		...
		--Delete
		UPDATE
			OW_LAO.DIM_PAYMENT_STATUS_MAPPING A	
		SET
			ACTIVE 				= FALSE,
			REPROCESSING 		= FALSE,
		...
		WHERE
			NOT EXISTS(
				SELECT 
					1
				FROM
					OW_LAO.TMP_PAYMENT_STATUS_MAPPING_MINIO B
				WHERE 
					A.PAY_STATUS 			= B.PAY_STATUS
					AND A.PO_STATUS  		= B.PO_STATUS
					AND A.PAY_DESCRIPTION 	= B.PAY_DESCRIPTION
		...
		--Atualização na tabela ods, onde somente ocorreu modificações
		UPDATE   
		...
		FROM   
			OW_LAO.ODS_PAYMENT_FUNNEL 				A
		JOIN   
			OW_LAO.DIM_PAYMENT_STATUS_MAPPING      	B ON B.PAY_STATUS = A.PAY_STATUS AND B.PO_STATUS = A.PO_STATUS AND B.PAY_DESCRIPTION = A.PAY_DESCRIPTION     
		WHERE  
			ACTIVE = TRUE
			AND REPROCESSING = TRUE; 	
	
		--Marca status de reprocessamento para FALSE		
		UPDATE   
			OW_LAO.DIM_PAYMENT_STATUS_MAPPING     
		SET 
			REPROCESSING = FALSE
		WHERE 
			REPROCESSING = TRUE;
	END;
	ELSEIF :CONSULTA = 'F_FUNNEL_PAYMENTS' THEN	
	BEGIN
		/*-------------------------------------------------------------------------------------------------*/
		--	
		...
		ORDER BY 
			IFNULL(A.UPDATED_TIMESTAMP,A.INSERT_TIMESTAMP) DESC
		LIMIT 100000;
