CREATE PROCEDURE  PROC_GSMARENA_MAPPING
...
INSERT INTO ODS_GSM_ARENA_STAGE
	 ...
	 , "mktcode"
	 , "mktname"
	 , MAX(case when trim("width_mm") ='-' then null else TO_DOUBLE("width_mm")end ) "width_mm"
	 , MAX(case when trim("height_mm")='-' then null else TO_DOUBLE("height_mm") end) "height_mm"
	 , MAX(case when trim("depth_mm") ='-' then null else TO_DOUBLE("depth_mm") end ) "depth_mm"
	...
	SELECT distinct "brand" "brand_nm", "modelcode" "model_code", "modelnm" "model_nm","mktcode" "mktcode","mktnm" "mktname"
		...
		, CASE
		  ...
		  END "depth_mm"
		, CASE
			WHEN ( UPPER("subcategory") LIKE '%DISPLAY SIZE%' AND "displaynm" in('Display_Display size','(Display) Size') )  THEN "value" 
		  END "display_size"
		, CASE
			WHEN ( UPPER("subcategory") LIKE '%RAM%' AND "displaynm" in('RAM_RAM Quantity','(Performance) RAM') ) THEN "value" 
		  END "ram_in_mb"
		, CASE
			WHEN ( UPPER("subcategory") LIKE '%INTERNAL%' AND "displaynm" in('Internal storage' ,'(Performance) Storage')) THEN "value" 
		  END "internal_storage"
		, CASE
			WHEN ( UPPER("subcategory") LIKE '%DUAL%' AND "displaynm" = 'Cellular_Dual SIM' ) THEN "value" 
		  END "dual_sim"
		, CASE
			WHEN ( UPPER("subcategory") LIKE '%CAMERA%' AND "displaynm" in ('Main camera_Resolution','(Camera) Rear Main Resol.') ) THEN "value" 
		  END "main_camera_mp"
		, CASE
			WHEN ( UPPER("subcategory") LIKE '%FRONT%' AND "displaynm" in('Front_Resolution','(Camera) Front Resolution') ) THEN "value" 
		  END "front_camera_mp"
		, CASE
			WHEN ( UPPER("subcategory") LIKE '%WEIGHT%' AND "displaynm" in('Weight_Weight in Grams','(Design) Weight (g)') ) THEN "value" 
		  END "weight_gramm"
		, CASE
			WHEN ( UPPER("subcategory") LIKE '%RESOLUTION%' AND "displaynm" in ('Resolution_Horizontal','(Display) Resolution H')) THEN "value" 
		  END "resolution_wid"
		, CASE
			WHEN ( UPPER("subcategory") LIKE '%RESOLUTION%' AND "displaynm" in('Resolution_Vertical','(Display) Resolution V') ) THEN "value" 
		  END "resolution_hig"
		, CASE
			WHEN ( UPPER("subcategory") LIKE '%CAPACITY%' AND "displaynm" in ('Battery_Capacity','(Performance) Battery Capa.') ) THEN "value" 
	 ...
	 OR ( UPPER("subcategory") LIKE '%DISPLAY SIZE%' AND "displaynm" in('Display_Display size','(Display) Size')) -- Display size
	 OR ( UPPER("subcategory") LIKE '%RAM%' AND "displaynm" in('RAM_RAM Quantity','(Performance) RAM') ) -- RAM
	 OR ( UPPER("subcategory") LIKE '%INTERNAL%' AND "displaynm" in('Internal storage' ,'(Performance) Storage') ) -- Internal Storage
	 OR ( UPPER("subcategory") LIKE '%DUAL%' AND "displaynm" = 'Cellular_Dual SIM' ) -- Dual SIM
	 OR ( UPPER("subcategory") LIKE '%CAMERA%' AND "displaynm" in ('Main camera_Resolution','(Camera) Rear Main Resol.') ) -- Rear Camera Resolution
	 OR ( UPPER("subcategory") LIKE '%FRONT%' AND "displaynm" in('Front_Resolution','(Camera) Front Resolution')  ) -- Front Camera Resolution
	 OR ( UPPER("subcategory") LIKE '%WEIGHT%' AND "displaynm"  in('Weight_Weight in Grams','(Design) Weight (g)') ) -- Weight
	 OR ( UPPER("subcategory") LIKE '%RESOLUTION%' AND "displaynm" in ('Resolution_Horizontal','(Display) Resolution H') ) -- Display Resolution Horizontal
	 OR ( UPPER("subcategory") LIKE '%RESOLUTION%' AND "displaynm" in('Resolution_Vertical','(Display) Resolution V') ) -- Display Resolution Vertical
	 OR ( UPPER("subcategory") LIKE '%CAPACITY%' AND "displaynm" in ('Battery_Capacity','(Performance) Battery Capa.') ) -- Battery Capacity
