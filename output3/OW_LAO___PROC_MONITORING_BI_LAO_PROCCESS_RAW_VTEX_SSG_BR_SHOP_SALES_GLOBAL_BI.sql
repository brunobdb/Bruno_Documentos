CREATE  procedure ow_lao.proc_monitoring_bi_lao_proccess_raw_vtex_ssg_br_shop_sales_global_bi
 LANGUAGE SQLSCRIPT AS
 
 begin
    declare default_searching_timestamp timestamp = '2024-08-01';
    declare start_searching_timestamp   timestamp = null;
    declare end_searching_timestamp     timestamp = null;
    
    select case 
                when add_days(current_timestamp, -3) < :default_searching_timestamp
                then :default_searching_timestamp
                else add_days(current_timestamp, -3)
            end
 ...
 left join OW_LAO.TF_D2C_PO_VTEX_TRADE_IN                   d on d.order_id       = a.order_id
     ...
     where 1 = 1
       --and a.order_id in ('1452295390525-01')
