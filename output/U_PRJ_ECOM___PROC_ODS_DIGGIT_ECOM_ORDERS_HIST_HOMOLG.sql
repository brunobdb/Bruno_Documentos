CREATE PROCEDURE u_prj_ecom.proc_ods_diggit_ecom_orders_hist_homolg
 ...
 begin
        ...
        select :order_creation_min as order_creation_min
          ...
          drop table u_prj_ecom.tmp_diggit_ecom_orders_ods_hist_homolg;
          
        create column table  u_prj_ecom.tmp_diggit_ecom_orders_ods_hist_homolg
...
select a.order_code
 ...
 where rn = 1
           ...
           select cast(min(order_creation_date) as date)  as order_creation_min
             ...
             from u_prj_ecom.tmp_diggit_ecom_orders_ods_hist_homolg
;            
        ...
        orders_codes_cancelleds =   select distinct 
                                     ...
                                     where order_creation_date between :order_creation_min and :order_creation_max
                                       and not exists(
                                                select 1
                                                  from u_prj_ecom.tmp_diggit_ecom_orders_ods_hist_homolg aa
                                                 where aa.order_code = a.order_code
                                                   and aa.sku        = a.sku
                                                   and aa.store_name = a.store_name
                                           );
               
         update u_prj_ecom.ods_diggit_ecom_orders_hist_homolog a
          ...
          merge into u_prj_ecom.ods_diggit_ecom_orders_hist_homolog  a
               using u_prj_ecom.tmp_diggit_ecom_orders_ods_hist_homolg  b on b.order_code = a.order_code
                                                          and b.sku        = a.sku
